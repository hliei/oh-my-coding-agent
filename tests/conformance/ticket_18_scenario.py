...
async def _precomputed(root: Path) -> dict[str, object]:
    ...
    previous_hold = mutation_queue.after_queue_hold
    setattr(mutation_queue, "after_queue_hold", hold)
    try:
        session = (
            await createAgentSession(
                CreateAgentSessionOptions(
        ...
        setattr(mutation_queue, "after_queue_hold", previous_hold)
        setattr(httpx, "AsyncClient", original)
...
async def _serialize(root: Path) -> dict[str, object]:
    ...
    setattr(mutation_queue, "after_queue_hold", gated)
    transport = _ScriptedTransport(
        _tool_calls_sse(
            (
                "call-a",
    ...
    try:
        ...
        setattr(mutation_queue, "after_queue_hold", original_hold)
...
async def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as raw_root:
        ...
        actual = {
            ...
            "reference.edit-mutation-serialization": await asyncio.wait_for(
