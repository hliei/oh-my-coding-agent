...
**What to build:** Demonstrate the canonical v0 Product Journey through public `AgentSession` in a controlled existing repository. The Agent inspects context, performs a bounded mutation with built-in Tools, runs the applicable named check after the final mutation, and truthfully reports the change and verification result while preserving all durable, failure, cancellation, and trust boundaries already established.
...
- [ ] Completion, disposal, exact-id recovery, and a later correction prompt preserve the complete durable conversation and rebind current resources/authentication.
...
- [ ] Matrix/corpus journey rows connect every public observation/effect to its decision authority and run only through the installed AgentSession seam.
