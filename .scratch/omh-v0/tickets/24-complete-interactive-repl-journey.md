...
- [ ] The REPL emits one safe `session <ID> new|recovered` identity record for default, path/id, recent, open-or-create, or in-memory selection, owns one lifetime input reader, and presents exact idle chrome.
...
- [ ] Real PTY installed-command tests cover trust, persistent and `--no-session` startup, transcript bytes/order, negative Tool Outcome labeling, busy input, multiple Runs, I/O failure, and Matrix/corpus evidence.
