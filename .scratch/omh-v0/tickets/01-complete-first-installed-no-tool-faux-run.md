# 01 — Complete the first installed no-Tool Faux Run

**What to build:** Ship the smallest complete installed-artifact tracer for omh: one atomic Distribution exposes all three Public Import Packages, a caller registers the deterministic Faux Provider, and `runAgentLoop` completes one no-Tool text Run with ordered public events and one immutable terminal result. This slice establishes packaging, locked development inputs, the initial public seam, and the first conformance authority without pulling later stream, Tool, AgentSession, or Command Mode behavior forward.
