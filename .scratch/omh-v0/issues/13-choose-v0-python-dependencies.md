...
- Standard-library `json`, byte/file I/O, and `pathlib`/`os` support the caller-visible `SessionManager`'s local append-only JSONL and in-memory modes. `SessionManager`, Session paths, the JSONL carrier, and tree operations are public as fixed by ticket 08; persistence, parsing, and indexing helpers remain private. No `sqlite3` Session store, SQLAlchemy, `aiosqlite`, ORM or migration framework, arbitrary storage Adapter, serializer/service, or Pi-file compatibility library is added.
